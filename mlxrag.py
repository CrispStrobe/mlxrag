...
class QdrantManager:
    ...
    def search_v2(self, query: str, search_type: str = "hybrid", limit: int = 10,
                processor: Any = None, prefetch_limit: int = 50, fusion_type: str = "rrf",
                relevance_tuning: bool = True, context_size: int = 300, 
        ...
        try:
            ...
            dense_model_id = getattr(processor, 'dense_model_id', self.dense_model_id)
            sparse_model_id = getattr(processor, 'sparse_model_id', self.sparse_model_id)
            dense_vector_name = f"dense_{dense_model_id.replace('-', '_').replace('/', '_')}"
            sparse_vector_name = f"sparse_{sparse_model_id.replace('-', '_').replace('/', '_')}"
            
            if self.verbose:
                print(f"Using vector names: {dense_vector_name} (dense) and {sparse_vector_name} (sparse)")
            
            # Skip relevance tuning for compatibility
            if relevance_tuning and self.verbose:
                print("RrfParams not available in this Qdrant client version. Skipping relevance tuning.")
            
            # Define a safe getter function for score
            def get_score(p):
                if hasattr(p, "score"):
                    return getattr(p, "score", 0)
                return 0
                
            # For preview purposes, we'll need to fetch all results first and then filter out tiny chunks
            
            # ------------------------------------------------
            # 1) DENSE-ONLY VECTOR SEARCH
            # ------------------------------------------------
            if search_type == "vector":
                ...
                try:
                    ...
                    # We need to handle score_threshold manually since some clients don't support it
                    # First get more results than needed
                    extra_limit = limit * 3  # Get 3x as many results to filter later
                    
                    # Try different API approaches
                    try:
                        # Modern approach with query_points
                        results = self.client.query_points(
                            collection_name=self.collection_name,
                            query=query_vector.tolist(),
                            using=dense_vector_name,
                        ...
                        if self.verbose:
                            print(f"Vector search using query_points returned {len(points)} results")
                        ...
                        try:
                            ...
                            if self.verbose:
                                print(f"Vector search using search method returned {len(points)} results")
                                
                        except Exception as e2:
                            # Try even older approach without named vectors
                            try:
                                results = self.client.search(
                                    collection_name=self.collection_name,
                                    query_vector=query_vector.tolist(),
                                ...
                                if self.verbose:
                                    print(f"Vector search using legacy method returned {len(points)} results")
                                    
                            except Exception as e3:
                                return {"error": f"All vector search methods failed: {e3}"}
                    
                    # Apply score threshold manually if needed
                    if score_threshold is not None:
                        points = [p for p in points if get_score(p) >= score_threshold]
                    
                    # Take only up to the requested limit
                    points = points[:limit]
                    
                except Exception as e:
                    return {"error": f"Error in dense vector search: {str(e)}"}
                
            # ------------------------------------------------
            # 2) SPARSE-ONLY SEARCH
            # ------------------------------------------------
            elif search_type == "sparse":
                print("=== Entering sparse-only search mode ===")
                if processor is None:
                    return {"error": "Sparse search requires an embedding model"}

                # Provide debug info on the vector name and collection
                if self.verbose:
                    print(f"[DEBUG] We'll search with vector name '{sparse_vector_name}' in collection '{self.collection_name}'")
                ...
                try:
                    ...
                    if self.verbose:
                        print(f"[DEBUG] Created SPLADE-like sparse vector for query '{query}':")
                        print(f"        Non-zero terms: {len(sparse_indices)}")
                        if len(sparse_indices) > 0:
                            print(f"        e.g. indices[:5]={sparse_indices[:5]}")
                            print(f"        e.g. values[:5]={sparse_values[:5]}")

                    # Perform sparse search with the same vector name used during indexing
                    try:
                        response = self.client.query_points(
                            collection_name=self.collection_name,
                            query=sparse_vector,
                        ...
                        if self.verbose:
                            print(f"[DEBUG] Qdrant returned {len(points)} points for sparse query.")
                            for i, pt in enumerate(points[:3]):
                                print(f"    Example result #{i+1}: id={pt.id}, score={pt.score:.4f}, "
                                    f"payload keys={list(pt.payload.keys())}")

                    except Exception as e:
                        if self.verbose:
                            print(f"[ERROR] Error in sparse search query: {e}")
                        ...
                        try:
                            points = self.client.search(
                                collection_name=self.collection_name,
                                query_vector=vector_dict,
                                limit=limit * 3,
                                with_payload=True
                            )
                        except Exception as e2:
                            print(f"[ERROR] Alternative sparse search also failed: {e2}")
                            return {"error": f"All sparse search methods failed: {e2}"}
                    ...
                    if self.verbose:
                        print("[ERROR] Exception in sparse search block:")
                        import traceback
                        traceback.print_exc()
            ...
            # ------------------------------------------------
            # 3) KEYWORD SEARCH - using filter-based approach without requiring dense embeddings
            # ------------------------------------------------
            elif search_type == "keyword":
                try:
                    # Create a payload filter for text matches
                    ...
                    # Array to store our converted results with scores
                    search_results = []
                    ...
                    try:
                        ...
                        try:
                            ...
                            raw_points = results[0]  # First element contains points
                        except Exception as e:
                            if "scroll_filter" in str(e):
                                # Try older API without scroll_filter
                                results = self.client.scroll(
                                    collection_name=self.collection_name,
                                    filter=query_filter,
                                    limit=limit * 3,
                                ...
                                raw_points = results[0]
                            else:
                                raise e
                        
                        # Convert Record objects to dictionaries we can modify
                        for point in raw_points:
                            # Extract needed data from the Record
                            point_dict = {
                                "id": point.id,
                                "payload": getattr(point, "payload", {}),
                                "score": 0.0  # Default score
                            }
                            search_results.append(point_dict)
                            
                    except Exception as e:
                        if self.verbose:
                            print(f"Error using scroll for keyword search: {e}")
                        ...
                        try:
                            ...
                            if self.verbose:
                                print(f"Error using query_points for keyword search: {e2}")
                                
                            # Method 3: Last resort with dummy vector search
                            try:
                                # Get vector size
                                vector_info = self.client.get_collection(self.collection_name)
                                vector_size = 384  # Default fallback size
                                
                                try:
                                    vector_params = vector_info.config.params.vectors
                                    vector_names = list(vector_params.keys()) if hasattr(vector_params, "keys") else []
                                    if vector_names:
                                        first_vector = vector_params[vector_names[0]]
                                        if hasattr(first_vector, "size"):
                                            vector_size = first_vector.size
                                except Exception as size_err:
                                    if self.verbose:
                                        print(f"Could not determine vector size: {size_err}. Using default.")
                                ...
                                # Convert to dictionaries, preserving original scores
                                for point in raw_points:
                                    point_dict = {
                                        "id": point.id,
                                        "payload": getattr(point, "payload", {}),
                    ...
                    # If we have no results, return empty results
                    ...
                    # Score based on keyword match quality
                    for point in search_results:
                        if point["payload"] and "text" in point["payload"]:
                            ...
                            if query.lower() in text:
                                # Exact match score
                                exact_score = 0.9
                                # Boost score based on match prominence (earlier = better)
                                position = text.lower().find(query.lower()) / max(1, len(text))
                                position_boost = 1.0 - min(0.5, position)
                                point["score"] = min(1.0, exact_score + position_boost * 0.1)
                            else:
                                ...
                                if term_count > 0:
                                    term_ratio = term_count / max(1, len(query_terms))
                                    point["score"] = 0.5 + (term_ratio * 0.4)  # Max 0.9 for all terms
                                else:
                                    point["score"] = 0.1  # Minimal matching score
                        else:
                            point["score"] = 0.05  # Default minimal score for valid results
                    
                    # Sort by score
                    search_results.sort(key=lambda p: p["score"], reverse=True)
                    
                    # Apply score threshold if needed
                    if score_threshold is not None:
                        search_results = [p for p in search_results if p["score"] >= score_threshold]
                    
                    # Limit results
                    search_results = search_results[:limit]
                    
                    # Now convert our search_results to a format compatible with other search types
                    points = []
            ...
            # ------------------------------------------------
            # 4) HYBRID SEARCH - Use vector search with post-processing
            # ------------------------------------------------
                ...
                try:
                    ...
                    if self.verbose:
                        print(f"Using dense vector ({len(query_vector)} dims) and sparse vector ({len(sparse_indices)} non-zero terms)")
                    
                    # Try using the prefetch/fusion approach if supported
                    try:
                        ...
                        if self.verbose:
                            print(f"Fusion-based hybrid search failed: {e}")
                            print("Falling back to dense search with post-processing")
                        
                        # Fallback to dense search with keyword boosting
                        try:
                            # Get results using dense vector
                            results = self.client.search(
                                collection_name=self.collection_name,
                                query_vector=(dense_vector_name, query_vector.tolist()),
                                limit=prefetch_limit,
                            ...
                            # Try sparse search as well if possible
                            sparse_results = []
                            try:
                                sparse_response = self.client.search(
                                    collection_name=self.collection_name,
                                    query_vector=(sparse_vector_name, sparse_vector),
                                    limit=prefetch_limit,
                                    with_payload=True
                                )
                                sparse_results = sparse_response
                            except Exception as se:
                                if self.verbose:
                                    print(f"Sparse part of hybrid search failed: {se}")
                            ...
                            for item in sparse_results:
                                if hasattr(item, "id") and item.id not in result_ids:
                                    combined_results.append(item)
                            
                            # Apply keyword boosting for hybrid search
                            for i, point in enumerate(combined_results):
                                if hasattr(point, "payload") and point.payload and "text" in point.payload:
                                    text = point.payload["text"].lower()
                                    # Check for query terms
                                    if query.lower() in text:
                                        # Boost score for exact match
                                        point.score = min(1.0, get_score(point) * 1.5)
                                    else:
                                        # Check for individual terms
                                        for term in query.lower().split():
                                            if len(term) > 2 and term in text:
                                                # Small boost for term match
                                                point.score = min(1.0, get_score(point) * 1.1)
                            
                            # Re-sort based on adjusted scores
                            points = sorted(combined_results, key=get_score, reverse=True)
                            
                            if self.verbose:
                                print(f"Manual hybrid search returned {len(points)} results")
                            
                        except Exception as e2:
                            return {"error": f"All hybrid search methods failed: {e2}"}
            ...
            # Update points to filtered list
            points = filtered_points[:limit]
            
            formatted_results = []
            for i, result in enumerate(points):
                payload = result.payload if hasattr(result, "payload") else {}
                score = get_score(result)  # Use our safe getter
                text = payload.get("text", "")
                
                # Create preview with much better context handling
                preview = self._create_smart_preview(text, query, context_size)
